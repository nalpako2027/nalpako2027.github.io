...
Anyway, "shrinking the sample a little" undersells what actually happens. Recently, I conducted an analysis using longitudinal High School Longitudinal Study (HSLS) data; running the regression model on complete cases alone cut the sample from over 19,000 students to just 6,368 — roughly a two-thirds reduction. That's not a minor footnote. It's the difference between a study and a shadow of one.
